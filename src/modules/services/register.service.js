...
class RegisterService {
  ...
  _validateApprovalData = async (decoded) => {
    ...
    if (company.employeeLimit) {
      ...
      if (currentCount >= company.employeeLimit) {
        throw new AppError(
          "บริษัทนี้มีจำนวนสมาชิกครบตามจำนวนที่กำหนดแล้ว",
          400
        );
      }
    }
  ...
  _processApproval = async (decoded, existingEmployee) => {
  ...
  _processRejection = async (decoded, reason) => {
    const { name, IDCard, companyId, lineUserId, start_date } = decoded;
  ...
  _linkRichMenuSafe = async (lineUserId) => {
    try {
  ...
  _sendLineNotificationSafe = async (lineUserId, message) => {
    try {
  ...
  createRegistrationRequest = async (userData) => {
    ...
    return {
      message:
        "คำขอลงทะเบียนของคุณถูกส่งไปยังผู้ดูแลระบบแล้ว กรุณารอการอนุมัติ",
      ...userData,
    };
  };
